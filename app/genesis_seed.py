...
async def run_genesis(
    ...
    """
    Convenience function to run genesis initialization.
    
    Creates mock miner IDs and runs the full initialization.
    In production, miner_ids come from OpenClaw agent registration.
    """
    miner_ids = [f"miner_{str(uuid4())[:8]}" for _ in range(num_miners)]

    config = SeedModelConfig(model_id=model_id)
    state = await genesis_initializer.initialize(
        model_config=config,
        miner_ids=miner_ids,
    )

    # Assign first round of tasks
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Initialize Genesis Seed")
    parser.add_argument("--model-id", default="resonant-seed-1b")
    parser.add_argument("--miners", type=int, default=10)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")

    result = asyncio.run(run_genesis(num_miners=args.miners, model_id=args.model_id))
